"""Register the standard pose pipeline in the local MLflow Model Registry.

Usage (from project root, with strikevision env active):

    python scripts/ml/register_pose_estimator.py
    python scripts/ml/register_pose_estimator.py --stage Staging
    python scripts/ml/register_pose_estimator.py --weight models/weights/pose_landmarker_lite.task
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=f"Register {POSE_ESTIMATOR_NAME} in the local MLflow Model Registry."
    )
    parser.add_argument(
        "--weight",
        type=Path,
    ...
    version = register_pose_estimator(weight, stage=args.stage, root=root)
    alias = args.stage.lower()
    print(f"Registered model: {POSE_ESTIMATOR_NAME}")
    print(f"  version: {version.version}")
    print(f"  alias:   {alias}")
    print(f"  stage:   {version.current_stage}")
    print(f"  run_id:  {version.run_id}")
    print(f"  source:  {weight or f'models/weights/{DEFAULT_POSE_WEIGHT_FILENAME}'}")
    print(f"  URI:     models:/{POSE_ESTIMATOR_NAME}@{alias}")
    print(f"  tracking URI: {tracking.as_uri()}")
    print()
    print("Description:")
    print(POSE_ESTIMATOR_DESCRIPTION)
    print()
    print("Inputs:")
